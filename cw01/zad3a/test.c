...
int main(int argc, char *argv[]) {
    if (argc == 1) {
        printf("You have to write what the program should do in command line arguments.\n");
        return 0;
    }
    if (!strcmp(argv[1], "cs")) {
...
//        char StaticArray[NoBlocks][SoBlocks];
        ...
        for (int i = 0; i < NoBlocks; i++) {
            ...
            *((char *) StaticArray + i * NoBlocks + SoBlocks - 1) = '\0';
//StaticArray[i][SoBlocks-1]='\0';
        }
        stopTime();
        printf("  Creating static array:\n    User: %ld.%lds\n    System: %ld.%lds\n    Real %ld.%.lds\n", endU.tv_sec,
               endU.tv_usec, endS.tv_sec, endS.tv_sec, endR.tv_sec, endR.tv_usec);
