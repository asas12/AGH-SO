...
void catchSIGUSR2(int signo, siginfo_t *info, void *ucontext) {
    ...
    //setting up waiting for SIGUSR1
    //int sig;
    //sigset_t sigmask;
    //sigemptyset(&sigmask);
    ...
    //sending conformation to client... sigusr2? so he can release FIFO and shm
    ...
    clock_gettime(CLOCK_MONOTONIC, &tp);
    printf("%d.%ld\tShaving client %d.\n", (int) tp.tv_sec, tp.tv_nsec, info->si_pid);

    //letting client know shaving is complete
    clock_gettime(CLOCK_MONOTONIC, &tp);
    printf("%d.%ld\tShaving client %d is done.\n", (int) tp.tv_sec, tp.tv_nsec, info->si_pid);
    //kill(info->si_pid, SIGUSR2);
...
//creating shared memory
    ...
    while (1) {
        ...
        if (k > 0) {
            ...
            //setting up receiving info from client
            //sigset_t sigmask;
            //sigemptyset(&sigmask);
            //sigaddset(&sigmask, SIGUSR1);
